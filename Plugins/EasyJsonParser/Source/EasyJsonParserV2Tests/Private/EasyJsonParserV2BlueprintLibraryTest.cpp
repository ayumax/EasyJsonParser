...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FEasyJsonParserV2BlueprintLibraryTest, "EasyJsonParser.V2.BlueprintLibrary", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

bool FEasyJsonParserV2BlueprintLibraryTest::RunTest(const FString& Parameters)
{
	...
	TestTrue("Original and loaded objects should be equal", UEasyJsonParserV2BlueprintLibrary::AreJsonObjectsEqual(JsonObject, LoadedObject));
	
	// Test debug functionality through Blueprint library
	UEasyJsonParserV2BlueprintLibrary::SetDebugMode(true);
	TestTrue("Debug mode should be enabled", UEasyJsonParserV2BlueprintLibrary::IsDebugMode());
	
	UEasyJsonParserV2BlueprintLibrary::SetDebugLogLevel(EEasyJsonParserV2DebugLogLevel::Detailed);
	TestEqual("Debug log level should be set correctly", UEasyJsonParserV2BlueprintLibrary::GetDebugLogLevel(), EEasyJsonParserV2DebugLogLevel::Detailed);
	
	// Test reading with debug enabled (should generate debug logs)
	FString DebugName = UEasyJsonParserV2BlueprintLibrary::ReadString(LoadedObject, "name", "");
	TestEqual("Debug read should work correctly", DebugName, FString("BlueprintUser"));
	
	// Test invalid access (should generate debug error)
	int32 InvalidValue = UEasyJsonParserV2BlueprintLibrary::ReadInt(LoadedObject, "nonexistent.path", -1);
	TestEqual("Invalid access should return default", InvalidValue, -1);
	
	// Disable debug mode
	UEasyJsonParserV2BlueprintLibrary::SetDebugMode(false);
	TestFalse("Debug mode should be disabled", UEasyJsonParserV2BlueprintLibrary::IsDebugMode());
